...
OSext.Data = function () {
	 ...
	 * 
	 * Spieltag 0 ist für den letzten Zat der Vorsaison reserviert.
	 * Damit gilt für die aktuelle Saison eine direkte Index-Zat-Zuordnung.
	 * Spieltage weiterer Folgesaisonen werden bei Bedarf angehängt.
	 */
	...
	/**
	 * Saisonpausenstatus; entspricht dem Zat 0
	 * (nach Zat 72 und Saisonwechsel, aber vor Zat 1)
	 */
	...
	/**
	 * Aktuelles {@link OSext.Team}. 
	 * Eine Referenz wird im Saisonplan zum aktuellen Termin abgelegt.
	 */
	this.team = new OSext.Team();
	
	/** 
	 * UI-Einstellungen ({@link OSext.Ansicht}).
	 * Enthält u.a. Termin-Auswahl und {@link OSext.Termin} für die Teamansicht.
	 */
...
OSext.Data.prototype = {
	...
	/**
	 * Initialisiert den aktuellen Zat von der Hauptseite; 
	 * zu diesem Zeitpunkt ist noch keine Saison bekannt.
	 * 
	...
	/**
	 * Gibt {@code true} zurück, wenn der Saisonplan zumindest einen verplanten
	 * Spieltag enthält.
	 * 
	 * @returns {Boolean}
	 */
	...
	/**
	 * Gibt {@code true} zurück, wenn die Saison bereits läuft bzw. 
	 * zumindest ein Spieltag gespielt wurde.
	 * 
	 * @returns {Boolean}
	 */
	 ...
	 * <p>
	 * Zusätzlich werden die Referenzen termin, und im aktuellen Spieltag die Referenzen 
	 * team und stadion gesetzt.
	 * 
	...
	/**
	 * Liefert den Spieltag zum {@link OSext.Termin}. Liegt der Termin in der Vorsaison, 
	 * oder nach der Folgesaison, wird {@code null} zurückgegeben.
	 * 
	...
	/**
	 * Liefert den Spieltag-Index zum {@link OSext.Termin}. Liegt der Termin in der Vorsaison, 
	 * oder nach der Folgesaison, wird {@code -1} zurückgegeben.
	 * 
	 * @param {OSext.Termin} termin
	 * @return Index (oder -1)
	 */
	getSpieltagIndex : function (termin) {
		...
		if (termin instanceof OSext.Termin && 
				termin.saison >= this.termin.saison && termin.saison <= this.termin.saison + (this.saisonpause ? 2 : 1)) {
	...
	/**
	 * Liefert den Saisonplan mit einer Saldoprognose anhand der Teamdaten 
	 * und der Parameter in {@code this.ansicht}.
	 * 
					...
					if (termin.getZats() < this.termin.getZats()) {
						...
						if (von && bis) {
							for (s = this.team.spieler.length - 1; s >= 0; s--) {
								team.spieler[s] = this.team.spieler[s].getSpieler(
									this.saisonplan.slice(von, bis), OSext.Prefs);
							}
						} else {
	...
	/**
	 * Initialisiert das Model (v.a. Spieler) mit Daten aus der Datenbank,
	 * und speichert die aktuell extrahierten Werte.
	 */
	...
	initRunden : function () {
		...
		var s, spieltag, rd, int = true,
			pokalrunden = ["1. Runde", "2. Runde", "3. Runde", 
			               "Achtelfinale", "Viertelfinale", "Halbfinale", "Finale"],
			pokalzats = [3, 15, 27, 39, 51, 63, 69];
		...
		for (s = 1, rd = 1; s <= OSext.ZATS_PRO_SAISON; s++) {
			...
			else if (spieltag.spielart == OSext.SPIELART.OSC || spieltag.spielart == OSext.SPIELART.OSCQ || 
					spieltag.spielart == OSext.SPIELART.OSE || spieltag.spielart == OSext.SPIELART.OSEQ) {
	...
	/**
	 * Liefert ein {@code OSext.Team} mit der angegebenen Id aus einer 
	 * Liste von externen Teams. Falls das Team noch nicht existiert,
	 * wird ein neues Team erstellt und in der Liste gespeichert. 
	 * 
	 * @param {Number} id
	 */
